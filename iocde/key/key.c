...
unsigned char keyscan()
{
    ...
    if(!KEY1) keynumber = 1;								//！！！修改此处，可以增加/减少按键数量，如果按下为高电平要改为if(!KEY1) keynumber = 1;
        ...
        case KEY_STATE_2: 											//按键状态2：判定按键有效的种类：是单击，双击，还是长按
        {
            if(key_press) 												//如果按键在 设定的双击时间内释放，则判定为单击/双击
                ...
                if(key_time_double >= DOUBLE_KEY_TIME && key_time_double < LONG_KEY_TIME) 	//达到双击设定时间后
                    ...
                    else if (key_press_nums ==3 && new_keynum == keynum)key_return = 4 + new_keynum*10;
                    else if (key_press_nums ==4 && new_keynum == keynum)key_return = 5 + new_keynum*10;
...
//        }break;
/***长按每固定时间返回一次键值****/		
		  case KEY_STATE_3:  // 长按触发后，等待释放或重复触发
		 { 
			if (!key_press)  // 持续按下
			{
				key_time++;  // 每次调用加 1，相当于10ms

				if (key_time >= REPEAT_KEY_TIME)  // 每 200ms 触发一次
				{
					key_return = Long_click + new_keynum * 10;  // 再次返回长按事件
					key_time = 0;  // 重新计时
				}
			}
			else  // 松手
