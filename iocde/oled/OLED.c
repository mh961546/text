...
void OLED_Init(void)
{
	...
	OLED_WriteCommand(0x3F); // 0x0E~0x3F 0.96:0x3F 0.91:0x1f
	...
	OLED_WriteCommand(0x12); // 0.96:0x12 0.91:0x02
	...
	OLED_WriteCommand(0xA8); // 设置多路复用率
	OLED_WriteCommand(0x1F); // 0x0E~0x3F 0.96:0x3F 0.91:0x1f
	...
	OLED_WriteCommand(0xDA); // 设置COM引脚硬件配置
	OLED_WriteCommand(0x02); // 0.96:0x12 0.91:0x02
	...
	while (String[i] != '\0') // 遍历字符串
	{
		...
		else // 否则，即多字节字符
		{
			...
			if (FontSize == OLED_8X16) // 给定字体为8*16点阵
			{
				...
				OLED_ShowImage(X + XOffset, Y, 16, 16, OLED_CF16x16[pIndex].Data16x16);
				XOffset += 16;
			}
			else if (FontSize == OLED_C_12X12) // 12x12字体
			{
				// 在12x12字模库中查找对应字符
				...
				// 显示12x12字模
				OLED_ShowImage(X + XOffset, Y, 12, 12, OLED_CF12x12[pIndex].Data12x12);
				XOffset += 12; // 更新X坐标偏移量
			}
		}
	}
